...
class EditTariffAttachment extends Component {
    inputStyle = "input input_weight";

    constructor(props) {
        super(props);
        let dateTimeArray = this.props.attachment.startDateTime.split('T');
        let dateArr = dateTimeArray[0].split('-');
        let clearDate = `${dateArr[2]}-${dateArr[1]}-${dateArr[0]} ${dateTimeArray[1]}`;
        this.state = {
            languageId: 0,
            successModal: false,
            errorModal: false,
    ...
    render() {
        return (
            <div className="form">
                <form>
                    <div className="caption">{window.pageContent['edit_tariff_attachment_header'][this.props.currentLanguage]}</div>
                        ...
                        <div className="select">
                            <select id="tariff__select">
                                {this.state.tariffs.map((tariff) =>
                                    <option selected={this.props.attachment.tariffName === tariff.name} key={tariff.id} value={tariff.id}>{tariff.name}</option>
                                )}
                            </select>
                        </div>
                    </div>
                        ...
                        <div className="select">
                            <select id="terminal__select">
                                {this.state.terminals.map((terminal) =>
                                    <option selected={this.props.attachment.terminalId === terminal.id} key={terminal.id} value={terminal.id}>{terminal.id}</option>
                                )}
                            </select>
                        </div>
                    </div>
                        ...
                        <div className="select">
                            <select id="cellparam__select">
                                {this.state.cellParams.map((cellParam) =>
                                    <option selected={this.props.attachment.cellParameterName === cellParam.name} key={cellParam.id} value={cellParam.id}>{cellParam.name}</option>
                                )}
                            </select>
                        </div>
                    </div>
                ...
                <ModalWindow
                    value="Ok"
                    textTitle={window.pageContent['success_edit_attachment'][this.props.currentLanguage]}
...
export default EditTariffAttachment;