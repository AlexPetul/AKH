...
class EditTariff extends Component {
    ...
    constructor(props) {
        super(props);
        let clearInterval = this.props.tariff.clearInterval;
        let intervalValue = Number(clearInterval.split(' ')[0]);
        ...
        this.state = {
            ...
            name: new ValidationInput([new Rule(TypeOfRule.REQUIRED, window.pageContent['name_error'][currentLanguage])],
                true, this.props.tariff.name),
            interval: new ValidationInput([new Rule(TypeOfRule.REQUIRED, window.pageContent['interval_error'][currentLanguage]),
                new Rule(TypeOfRule.INTERVAL, window.pageContent['invalid_diapason_error'][currentLanguage])], true, intervalValue),
            price: new ValidationInput([new Rule(TypeOfRule.REQUIRED, window.pageContent['price_error'][currentLanguage])],
                true, this.props.tariff.price),
            description: new ValidationInput([], true, this.props.tariff.description),
        }
    }
        ...
        if (validationResult.isValid) {
            ...
            if (!langFound) {
                ...
                this.setState({showLoader: true}, () => {
                    ...
                    if (Number(clearInterval) > 0) {
                        if (this.state.price.value > 0) {
                            API.post('updateTariff', {
                                ...
                                .then(response => {
                                    if (response.data.errorCode === 1) {
                                        this.setState({
                                            errorText: window.pageContent['invalid_data'][this.props.currentLanguage],
                                            errorModal: true,
                                            showLoader: false
                                        });
                                    } else {
                            ...
                            this.setState({
                                errorText: window.pageContent['price_error'][this.props.currentLanguage],
                                errorModal: true,
                                showLoader: false
                            })
                        }
                        ...
                        this.setState({
                            errorText: window.pageContent['interval_error'][this.props.currentLanguage],
                            errorModal: true,
                            showLoader: false
                        })
                    }
    ...
    render() {
        ...
        return (
            <div className="form">
                <form>
                    <div className="caption">Редактировать тариф</div>
                    <Input
                        label={window.pageContent['name_field'][this.props.currentLanguage]}
                        name="name"
                        maxLength={30}
                        value={name.value}
                    ...
                    <div className="form__input">
                        <div className="wrap-label">
                            <div className="label-input">
                                {window.pageContent['text_field'][this.props.currentLanguage]}
                            </div>
                            <div className="lang">
                                <svg onClick={e => {
                                    e.preventDefault();
                        ...
                        {this.state.languageList.map((language, index) =>
                            <div className="form__input" id={"lang__input__" + language.id} key={index}>
                                {this.props.tariff.names.map((name) =>
                    ...
                    <IntervalInput
                        label={window.pageContent['interval_field'][this.props.currentLanguage]}
                        name="interval"
                        intervalType={this.props.tariff.clearInterval}
                        maxLength={30}
                    ...
                    />
                    <Input
                        label={window.pageContent['price_field'][this.props.currentLanguage]}
                        name="price"
                        maxLength={30}
                        value={price.value}
                        onChange={this.handleChange}
                    ...
                    />
                    <Textarea
                        label={window.pageContent['description_field'][this.props.currentLanguage]}
                        name="description"
                ...
                <ModalWindow
                    textTitle="Тариф успешно изменен!"
                    value="Ok"
